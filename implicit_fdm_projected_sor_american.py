...
def american_option_price_implicit_projected_sor(K, T, sigma, r, q, S, option_type):
    """
    Calculates the price of an American call or put option using the Implicit FDM and Projected SOR method.
    ...
    # Solve for the option price at each time step using Projected SOR
    for j in range(N - 1, -1, -1):
        l = np.zeros(M - 1)
        l[0] = alpha[1] * (f[0, j] + f[0, j + 1])
        l[-1] = gamma[M - 1] * (f[M, j] + f[M, j + 1])

        # Projected SOR iteration loop
        for _ in range(max_iterations):
            ...
            f[1:M, j] = (1 - omega) * prev_f + omega * f[1:M, j]  # relaxation
